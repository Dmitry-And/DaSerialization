...
public class ContainerRefWithIdDrawer : PropertyDrawer
{
    ...
    private Cached<TextAsset> __text = new Cached<TextAsset>();
    private Cached<int> __textCount = new Cached<int>();
    ...
    private void UpdateContainer(SerializedProperty property)
    {
        ...
        if (_container == null
            | __text.Update(text)
            | __textCount.Update(text.bytes.Length))
        {
    ...
    public override void OnGUI(Rect pos, SerializedProperty property, GUIContent label)
    {
        ...
        if (idToDelete != 0)
        {
            var cRef = property.serializedObject.targetObject.GetField<ContainerRefWithId>(property.propertyPath);
...
public class ContainerRefDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect pos, SerializedProperty property, GUIContent label)
    {
        ...
        if (idToDelete != 0)
        {
            var cRef = property.serializedObject.targetObject.GetField<ContainerRef>(property.propertyPath);
            cRef.Remove(idToDelete, typeToDelete, true);
        }
